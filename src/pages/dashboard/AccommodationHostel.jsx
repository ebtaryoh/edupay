...
export default function AccommodationHostel() {
    ...
    async function loadBills() {
      try {
        ...
        if (
          ...
          setError("Failed to load Accommodation/Hostel fees.");
        }
      } finally {
        setLoading(false);
      }
        ...
        <div className="mt-4 space-y-4 max-w-[720px]">
          {loading ? (
            ...
            <div className="py-10 text-center bg-[#F6F7FF] rounded-2xl text-[#9AA0B4]">
              <p>No Accommodation/Hostel fees found.</p>
            </div>
          ) : (
            filteredBills.map((bill, index) => (
              <FeeRow
                key={bill.id || index}
                title={bill.feeName || bill.title || bill.description || "Hostel Fee"}
